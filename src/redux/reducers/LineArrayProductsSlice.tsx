import { LineArrayDataUtils, type LineArrayProductsData } from '@/schemas/LineArrayProductTypes';
import {
    ...
    extraReducers: builder => {
        ...
        builder.addCase(PRODUCT_LIST_S as string, (state, action: PayloadAction<unknown>) => {
            const response = action.payload as {
                data: LineArrayProductsData | LineArrayProduct[];
                total?: number;
                page?: number;
                limit?: number;
            };
            ...
            if (Array.isArray(response.data) && response.data.length > 0) {
                // Check if first item has categoryName and subCategories (hierarchical)
                const firstItem = response.data[0] as LineArrayProductsData[0];
                if (firstItem.categoryName && firstItem.subCategories) {
                    // Hierarchical data
                    state.categoriesData = response.data as LineArrayProductsData;
                    state.productList = LineArrayDataUtils.flattenProducts(
                        response.data as LineArrayProductsData,
                    );
                } else {
                    // Flat data (backward compatibility)
                    state.productList = response.data as LineArrayProduct[];
            ...
            state.pagination.totalItems = response.total ?? state.productList.length;
...
// Selectors
export const selectProductList = (state: { lineArrayProducts: LineArrayProductsState }) =>
    state.lineArrayProducts.productList;
export const selectCategoriesData = (state: { lineArrayProducts: LineArrayProductsState }) =>
    state.lineArrayProducts.categoriesData;
export const selectProductInfo = (state: { lineArrayProducts: LineArrayProductsState }) =>
    state.lineArrayProducts.productInfo;
export const selectIsLoading = (state: { lineArrayProducts: LineArrayProductsState }) =>
    state.lineArrayProducts.isLoading;
export const selectError = (state: { lineArrayProducts: LineArrayProductsState }) =>
    state.lineArrayProducts.error;
export const selectFilters = (state: { lineArrayProducts: LineArrayProductsState }) =>
    state.lineArrayProducts.filters;
export const selectSearchQuery = (state: { lineArrayProducts: LineArrayProductsState }) =>
    state.lineArrayProducts.searchQuery;
...
export const selectPagination = (state: { lineArrayProducts: LineArrayProductsState }) =>
    state.lineArrayProducts.pagination;

// New selectors for hierarchical data
export const selectCategoryNames = (state: { lineArrayProducts: LineArrayProductsState }) =>
    LineArrayDataUtils.getCategoryNames(state.lineArrayProducts.categoriesData);

export const selectSubCategoryNames = (state: { lineArrayProducts: LineArrayProductsState }) =>
    LineArrayDataUtils.getSubCategoryNames(state.lineArrayProducts.categoriesData);

export const selectTotalProductCount = (state: { lineArrayProducts: LineArrayProductsState }) =>
    LineArrayDataUtils.getTotalProductCount(state.lineArrayProducts.categoriesData);

export const selectProductsByCategory =
    (categoryName: string) => (state: { lineArrayProducts: LineArrayProductsState }) =>
        LineArrayDataUtils.getProductsByCategory(
            state.lineArrayProducts.categoriesData,
            categoryName,
        );

export const selectProductsBySubCategory =
    (subCategoryName: string) => (state: { lineArrayProducts: LineArrayProductsState }) =>
        LineArrayDataUtils.getProductsBySubCategory(
            state.lineArrayProducts.categoriesData,
            subCategoryName,
        );
